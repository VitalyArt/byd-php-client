...
| `VehicleState` | UNKNOWN=-1, STARTED=0, DRIVING=1, POWER_OFF=2 (helpers: isPowerOn/isPowerOff/isUnknown) |
| `ChargingState` | UNKNOWN=-1, NOT_CHARGING=0, CHARGING=1, NOT_CONNECTED=15                                |
...
- `oilEndurance < 0` → `null` (hybrid vehicles without oil data)
- `oilPercent` — raw value preserved as-is (no sentinel normalization)
- `ectValue < 0` → `null`
